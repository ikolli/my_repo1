...
Within this Worksheet, we will walk through a large set of Snowflake functionality covering key features like Zero Copy Cloning and Time-Travel to deliver on a Tasty Bytes business requirement.
--*/
...
---> set the Schema
SET schema_name = CONCAT(current_user(), '_INTRO_TO_DATA_TRANSFORMATIONS');
...
COPY INTO truck
FROM @blob_stage_csv/raw_pos/truck/;

-- confirm the Truck table has data
SELECT * FROM truck
LIMIT 10;
...
--> NOTE: a Warehouse isn't required in a Clone statement as it is handled via Snowflake Cloud Services
...
-- let's re-run our query from above
SELECT
    t.truck_id,
    t.year,
    t.make,
...
-- Let's now Swap our Development Table with the Production
ALTER TABLE truck_dev
    SWAP WITH truck;
